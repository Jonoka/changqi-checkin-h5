...
版本由 A0 核对当前官方支持与兼容性后锁定。仅两张业务表，InnoDB、utf8mb4；会话组件可另建技术表。服务器文件存持久目录，数据库只保存路径。不引入 Redis、对象存储、消息队列、微服务或复杂 CI。
...
前后端同域 HTTPS；Nginx 请求体上限与照片上限一致留余量（15 MiB 文件可设置约 20 MiB 请求限制）。`UPLOAD_DIR`、MySQL 数据、环境文件独立于构建包。沿用服务器已有进程管理和备份方式，不为本期另建运维平台。

部署前保留当前代码及数据库/照片备份；只回退代码不恢复旧游客数据。不得整库清理测试数据。生产修改、公众号菜单修改和数据清理分别需要明确授权。
...
| AC-13 | 重启/部署不丢记录和照片；生产禁用模拟；环境文件、照片和口令不入 Git | 部署/代码检查 |
...
本文件重整既有 PRD v1.1、TSD v1.1、Agent 计划 v1.2 和当前对话决策。本轮是文档整理，**未重新访问平台文档、未核验实际公众号权限或运行时版本**；以下为沿用的实施核对入口，不表示已经接入成功：
