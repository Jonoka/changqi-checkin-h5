...
`points[].image` 是地图地标、列表缩略图与详情共用的图片路径，统一通过 key 关联。可选 `imageAlt`、`photoTip` 为简短文字（保留配置兼容，`photoTip` 不在页面展示），`imagePosition` 为两个 0–100% 裁切焦点；`mapPosition:{x,y}` 为百分比示意坐标（x 15–85、y 12–88）。缺少手工位置时整组采用简单交错排布，不推断真实距离、GPS 或导航。名称、编号、路线和状态由 DOM/SVG 渲染；状态只按 `completedKeys` 的 key 判断。当前五处须使用各自可识别的概念插画，不冒称已核验实景或历史事实。公众号可配置 `wechat.officialAccountQr` 为本地真实二维码图片；未配置时明确搜索“印象芦苞”及菜单进入步骤，不生成替代假码。
