...
地图与地点插画恢复 `assets/reference/` 已入库 PNG 的颜色、笔触和建筑细节；先裁切并局部清理含 UI 的原图，独立保存可追溯的无 UI 图片母版，再由 sharp 派生运行资源，不再从手写 SVG 生成插画。地图中的村落、五处地标与中性步道同属图片层，不叠加旧地标卡片或 SVG 路线；名称、编号和完成状态为真实 HTML 按钮，状态只按 `completedKeys` 的 key 判断。底图按自身比例完整缩放，热点与图片共用定位容器；不随 N 拉长。`mapPosition:{x,y}` 为针对该底图核对的百分比标签锚点（x 15–85、y 12–88），没有锚点的新地点只在既有折叠列表访问，不自动占用其他地标；移除配置地点即移除其标签，不改变动态进度或扫码门禁。
