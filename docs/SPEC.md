...
## S07 · 只读统计、CSV 与受控照片导出

2026-09-25 用户确认扩展原“仅领取人数、不导出”规则；原版历史记录保留。复用 users/checkins，无新业务表、累计计数或索引迁移。`mountStats` 明确接收服务端唯一 activityConfig，必达 key 精确匹配、N 非空；不接收浏览器必达集合。

共同口径：已识别=users 身份数；参与=至少一个当前有效地点；完成=全部当前必达地点；已标记领取=claimed_at 非空；完成未领取=全部完成且 claimed_at 为空，不能直接用完成减全部领取；每地点为不同身份数（包含零人地点）；有效照片记录=当前地点 checkins 数。只识别/扫码、重复上传、替换不增加参与。历史无关 key 不计；缺图不撤销历史打卡。已领取但不完整单独警示且不修数据。BIGINT id 保持十进制字符串/CQ 精度。人数均指微信身份，不称独立核验的自然人数。

每人先聚合全部有效记录，再将 MIN(created_at) 作为首次参与、仅全部完成者的 MAX(created_at) 作为推导首次完成，claimed_at 作为首次领取；按北京时间 +08:00 日期汇总。原打卡时间不是重拍时间或原始拍摄时间。不制造 PV、每日活跃、分享/重拍次数；活动期间仍固定必达集合。

非公开 `STATS_EXCLUDE_CQ_FILE` 可指向负责人确认的 JSON 字符串数组（规范 CQ 编号），默认未设置即空名单。坏格式、重复项、非规范编号或指定文件读取失败报错；所有报告显示启用状态/匹配排除数，不泄露完整名单，不猜测或删除测试数据。

`/stats` 与固定 `/stats/export/{summary,points,daily}.csv` 先经既有 STATS_USER/STATS_PASSWORD Basic Auth 再查库；缺凭据503、错误凭据401、已认证未知报表404。生产仅经HTTPS。固定路由映射，不将参数拼 SQL/路径。报告共用短、同 connection 的 REPEATABLE READ / READ ONLY 一致快照，完成读取即结束事务并释放；失败不返回假0、成功空表或半张表，不使用行/表锁。普通刷新不读照片目录。不同请求标各自查询时间。

CSV 使用 UTF-8 BOM、中文表头、逗号/引号/换行转义和文本公式注入防护；数量为数值，CQ 为文本。每份含活动、规则版本、查询时间、时区和排除口径。网页三个 CSV 仅聚合数据，无逐游客资料。

CLI `npm run export:activity -- --out <私有新批次绝对目录> --scope participants [--dry-run]`，默认 participants，也支持 completed/claimed。必须显式 out，拒绝覆盖；环境由操作者显式提供，不自动读取应用 .env、不打印密钥、不建库/迁移/改权限。dry-run 只查人数、记录、可读性、字节估计和位置，不创建导出文件。业务 SQL 只读。

资料包：统计汇总/地点统计/每日统计（全活动口径），游客进度（CQ、完成数/N、首次参与/推导完成/领取时间），照片清单（CQ、key/名称、首次保存时间、快照/实际版本、相对路径、字节、SHA-256、导出时间、结果），异常清单和 export-info.json。info 记录批次、起始快照、导出起止、+08:00、scope、必达key/配置哈希、可核验源码指纹/可取得的提交标识、规则版本、选中人数、预期/成功/异常文件数与状态；取不到提交或镜像标识如实说明。客户包不含 OpenID、claimCode、Cookie、私有链接、绝对源路径、密钥、完整排除名单或数据库备份。

先读快照及清单并结束事务，再逐张新提交读取路径/revision、安全打开、流式复制已处理 JPEG 原字节并核对前后路径/版本。替换或消失用新提交读取最多3次，仍变化/缺失记异常；不混入快照后新增用户/记录，不在复制期持长事务。统计和稍后照片分别标时间版本。正式收尾建议负责人授权停止写入并等在途完成后导出，脚本不自动停活动。

来源只允许合法随机 basename，拒绝越界、符号链接和非普通文件；输出 CQ/key/安全化名称兼容Windows，真实路径在源码、公开目录、源uploads之外。仅新建私有批次，逐文件复制、预估空间；磁盘满、权限、断连和中断真实失败，仅清理本次临时文件，不删源图/其他批次。退出码0=完整、2=部分异常、1=致命失败；预期=成功+异常，致命写盘失败以未完成状态为准，不声称成功。下载到负责人电脑后用系统工具压缩；照片只用于本活动内部记录/核查，后续清理另行授权。
