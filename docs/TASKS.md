...
**当前轮：微信地点入口直达（2026-09-23，最新版本已部署，微信真机待验）。** 从实际干净 Windows / 远端 HEAD `260768c33c7753691c16c9ed97acb9b740ce479c` 接续 `feat/a1-wechat-scan`。提交前回读 main 仍为 `cc411965dd384e9608a80abf87a9e883eea8294d`，PR #1 Draft/Open、未合并、Review 为空，未在旧 SHA 上覆盖他人提交。最终 HEAD 以 Git/PR 回读为准，不写自引用 SHA。
...
- 服务器核验与运行时：`root@100.95.32.56` 可通过 Tailscale 连接；Alibaba Cloud Linux 3.2104 LTS 内核 `5.10.134-18.al8.x86_64`，`x86_64`，约 0.94 GiB RAM，根盘 40G/可用约 18G。Nginx 1.26.2、MySQL 服务 active，项目库建表成功；配置端口 3306、socket `/tmp/mysql.sock`、数据目录 `/www/server/data`。现有 Node 应用占用 3000，本项目 app 绑定 `127.0.0.1:3002`；Docker CE 26.1.3、containerd 1.6.32、Compose v2.27.0 已从阿里云镜像安装并设为开机启动，`docker version`、`docker info`、`docker ps` 已通过。最新 artifact 已上传至 `/opt/changqi-checkin-h5/releases/35836566244`，归档 SHA-256 校验为 `6e737d69b0120e15b20040c9f31eff151dbb8f64af522a545871de4324bc6ee9`，镜像已 `docker load`。`cq.fsxinhuo.cn` 已由宝塔 Nginx 反代到本项目并强制 HTTP→HTTPS，最新镜像 HTTPS 健康检查、活动接口和静态插画资源通过，证书有效期至 2026-12-20；未改其他站点、未清理生产数据。
...
- 当前接续：A5 本地回归通过、微信真机未验；最新提交 `260768c33c7753691c16c9ed97acb9b740ce479c` 已由 Actions run `35836566244` / attempt 1 构建、冒烟、导出并上传 artifact；A6.3 样张和自动解码完成、真实纸样未验；A6.2 已完成新 artifact 下载、校验、加载、幂等建表、容器替换和 Nginx HTTPS 检查。当前生产容器/HTTPS 已实测，微信 iOS/Android 真机、真实照片和纸样仍待验。
...
| A4 | 保留布局/UI，接入六张独立 PNG 插画 | A2、A3 | 本地接入/回归/目视通过；最新版本已发布，微信真机待验 |
...
发布边界：本次用户已明确授权对最新提交构建并更新生产。SSH 实测容器 `changqi-checkin-h5-app-1` 正运行 `changqi-checkin-h5:260768c33c77-35836566244-1`，发布目录 `build-info.json` 对应 source SHA `260768c33c7753691c16c9ed97acb9b740ce479c` / run `35836566244` attempt 1；OCI revision label 为空，SHA 来源明确为配套发布元数据，不冒称容器标签自证。当前镜像 ID 为 `sha256:e77690a712abcbe8645629b814204a58222ca7928eee362bb90723a7cd24cbe8`，容器 running；回环和公网 HTTPS `/health` 返回 database connected，`/api/activity`、`/q/p01`、插画资源、未登录 401、无效领取码 404、`/stats` 401 均通过。旧镜像 `f14430b7039d-35750893371-1` 与环境备份保留，可回退；未改公众号菜单、未清理生产数据，普通 Chrome 与模拟 SDK 不是微信真机结果。
...
最新生产核对为 `changqi-checkin-h5:260768c33c77-35836566244-1`，image ID `sha256:e77690a712abcbe8645629b814204a58222ca7928eee362bb90723a7cd24cbe8`；build-info source `260768c33c7753691c16c9ed97acb9b740ce479c` / run 35836566244 attempt 1，OCI revision label 为空，回环 health=database connected。本轮已触发 Actions、下载并校验 artifact、替换生产容器；旧镜像/环境备份保留，未清库或修改微信/正式地点码；真机仍待验。
...
| AC-13 | 最新版本构建/冒烟/下载/服务器部署通过，微信真机未验 | 生产禁模拟、开发身份不可转正式、MySQL 会话/照片/领取实际进程重启恢复已回归。Actions run `35836566244` / attempt 1、源码 `260768c33c7753691c16c9ed97acb9b740ce479c`：linux/amd64 镜像构建、临时 MySQL schema、`/health`、`/api/activity`、首页、`/q/p01`、无效领取 API 和生产模拟拒绝均通过；镜像内 Node `v24.21.0`、npm `11.19.0`，基础 digest 为 `sha256:5cbc7caba8c2c0f0bca675d1b61b9f2857e1cf1853c6164ee9dd409501a936e7`。artifact `changqi-image-260768c33c77-35836566244-1` 已下载到 `tmp/release-35836566244`，归档 SHA-256 为 `6e737d69b0120e15b20040c9f31eff151dbb8f64af522a545871de4324bc6ee9`；已上传并在服务器 `docker load` 成功。项目库幂等建表成功；容器 `changqi-checkin-h5:260768c33c77-35836566244-1` 运行于 `127.0.0.1:3002`，回环和公网 HTTPS `/health` 返回数据库 connected；Nginx HTTPS 反代、HTTP 301、最新活动接口/地点引导/插画、未登录 401、无效领取码 404 和 `/stats` Basic Auth 均通过。旧镜像与环境备份保留，可回退；微信真机、真实照片/领取和纸样仍未验证。 |
...
**当前接续（2026-09-23）：**六图 PNG 已实际提交，原 Draft PR #1 / feat/a1-wechat-scan 继续；新地图/五处详情/列表已接入，check 56/56、build、隔离 MySQL/Chrome 39/39、最终页面目视通过。新证据 `tmp/changqi_a1_test_232bffccbccc`，详细范围见本轮六图记录。用户随后明确授权对最新提交 `260768c...` 构建部署；Actions run `35836566244` 全部通过，artifact 已校验并替换生产容器，HTTPS 与关键只读接口通过。旧镜像/环境备份保留；微信真机、真实照片/领取和纸样仍待验。
