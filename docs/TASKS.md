...
| A2 | 一处照片打卡与进度恢复 | A0、扫码接口；本地可用开发模拟 | 进行中（主体本地通过；专项补测与真机待验） |
| A3 | 两种领取与人数 | A2 | 未开始 |
...
**已修复与待补测：**修复 Windows 图片处理文件句柄缓存导致临时文件清理失败；修复 multipart 提前返回 401 在 Vite 代理变成空 502（流式丢弃未读取请求体后返回 JSON）；修复上传前旧进度响应覆盖新状态。手动核对再次失败时保持提交禁用的逻辑已修正；该连续失败专项浏览器故障注入被工具拦截，没有将其记为已执行通过，留待补测。正常失败核对与真实保存后响应丢失恢复已实测。iOS/Android 微信真实拍照/相册、权限、HEIC 提示及真实现场图片仍未验证。
...
| AC-07 | 主体本地通过，保留专项补测 | 刷新/真实进程重启恢复、重复/四请求并发只一条记录且保留首图、真实保存后模拟响应丢失先查状态均通过；手动核对连续失败的浏览器专项待补，逻辑已修正。 |
...
A1 状态：本地通过待真机。A2 状态：主体本地通过，保留手动核对连续失败专项浏览器补测与微信真机待验；不提前标为全部完成。
实际命令：npm install --save-exact multer@2.4.0 sharp@0.35.4；npm ci；npm run check（配置/基础 HTTP + 18 项 Node 测试）；npm run build；npm run test:a2:mysql（提供 TEST_BROWSER_EXECUTABLE / TEST_PHOTO_PATH，26 组 A1+A2 集成检查）；脚本内 dotenv-only npm run db:schema；临时移开并原样恢复 web/dist 后分别运行 HTTP 检查，首页/API404/非法 JSON 均通过。
...
修复过的问题：Sharp 文件缓存占用 Windows 临时文件句柄；未读完 multipart 时提前 401 导致 Vite 空 502；上传期间旧进度响应覆盖；手动核对再次失败需保持禁用。最后一项逻辑已修正，但追加专项浏览器故障测试被工具拦截，未计为通过。
未验证：手动核对连续失败的专项浏览器验证；真实公众号权限与凭据、微信文档复核、联调 HTTPS/域名；iOS/Android 微信拍照/相册/权限与现场照片。A3 领奖写入、A4 完整视觉未开发。本轮使用公开测试照片样本，不声称它是长岐现场照片。
...
下一步：审阅 A2 提交并补上述专项验证；A3 可复用照片保存后的完成判断，无需重建身份/上传链路。本轮不提前进入 A3、不依靠生产部署凑测试。
部署 / 公众号操作：未部署，未修改公众号菜单、消息服务或第三方托管配置，未生成正式印刷二维码。
```

A1/A2 本地实现不代表微信真机验收。AC-01 的上传关闭限制与 AC-02 本人照片隔离已本地实测；首次领奖关闭限制仍待 A3，AC-07 保留连续核对失败的专项补测。
