...
| A4 | 保留布局/UI，接入地点插画与最终现场地图 | A2、A3 | 本地回归/三宽目视与生产部署通过；微信真机待验 |
...
| AC-12 | 最终地图本地/目视及生产资源通过；待微信真机 | 完整现场地图 PNG/WebP 已接入，页面无额外可见路线、兑奖处、编号或地点标签；五个透明热点、折叠列表、动态增删、无位置地点兜底、扫码资格不变及 320/390/430px 点击/比例/首屏均通过。生产最终地图返回 200 image/webp/560,002 B；不冒称微信真机已验。 |
...
**当前接续（2026-09-24）：**凭证照片展示、显式单张替换和增量迁移已实现；最终69项Node、56项MySQL/Chrome完整回归、构建与三宽目视通过，证据见第0节。交付沿原分支正常提交推送，实际结果以Git/PR回读为准，PR继续Draft/Open；没有转Ready/合并、Actions、部署或生产迁移。下轮只需针对新增照片核查/替换做用户页面确认与另获授权后的真机验收，不重建既有功能。最终地图原发布源 `7e792a31185ad2255e88bdeda53e421bfba88f95` / run `35962178962` attempt 1 保留，不因旧 docs-only 记录重复构建。既有微信 iOS/Android 授权、扫码、照片上传、领取及纸样由用户确认通过；新增能力独立验收。
