...
- 本轮推进 A4：地点名/插画/提示/地图位置统一按 `point.key` 读取配置；首页改为村落层次环境底图 + 独立地标插画 + SVG 弯曲路线 + DOM 标签的漫游地图，列表默认折叠，不再连续展开两份地点集合。详情成为独立 hash 视图，优先返回地图、地点名、对应插画、拍照提示、紧凑进度和当前主操作；最后一处完成从当前位置进入本人 `#claim` 凭证，不打开工作人员 `/r/` 页。上传入口保留真实 file input，派发页去掉古井装饰；公众号引导保留“印象芦苞”和菜单步骤，未配置真实公众号二维码时只提示名称搜索。A1–A3 服务端业务规则与领取幂等不变。
- 本轮实际验证：最终 `npm run check` 为 32/32，`npm run build` 成功，`git diff --check` 通过；独立回环 MySQL 8.4.9（127.0.0.1:3316）+ 本机 Chrome 的 A1–A4 完整回归 39 项通过。含 A2 连续核对失败锁定、A3 两路并发/统计保护和真实应用重启；A4 逐一验证五个 key 的名称/插画/提示/扫码一致、p03→p04→p05 不残留上站图片、地图查看不授予资格、N=6 重排与 N=4 删除、缺图无误导兜底、最后一处真实上传后本人凭证。最终留存 44 个页面状态 × 320/390/430 共 132 张 PNG，路线/地标/点击目标坐标断言无偏移；微信 SDK/网络/故障注入仍明确为模拟，数据库/文件/HTTP/Vue 页面真实执行。
...
**实际画面证据：**最终目录为 `tmp/changqi_a1_test_a86ad91a5a9b/`；`index.html` 是可直接本地打开的视觉复核页，内含原 `assets/reference/map.png` 与本轮 390px 实际地图并排对照，`a4-screenshots.json` 记录分支、基准 HEAD、运行时源码/素材指纹及截图元数据。共 44 个状态 × 320/390/430 = 132 张整页 PNG，覆盖首页折叠/展开、五地点查看与扫码就绪、选图/保存中/结果未知/成功、动态 N/重排/缺图、最后一处成功、本人凭证、全部完成、已领取、工作人员派发、引导/授权/关闭/统计等。另有 `a4-map-only-*.png` 和关键状态复核拼图。已实际打开复核页目视：地图有地标、弯曲路线、村落/树木/山坡层次；五详情分别可识别为葫芦/祠堂/古井/榕树/山顶，上传与领奖按钮未遮挡关键内容。测试照片仍是生成夹具，不冒充现场拍摄。
...
| AC-12 | 本地通过，待真机/用户最终确认 | A4 最终 Chrome 44 状态 × 320/390/430 共 132 张整页图；无横向溢出、可见按钮触控尺寸与图片加载通过，地图路线/地标/点击目标同坐标无偏移。五处 key 独立插画、折叠列表、详情层级、动态 N/重排、缺图兜底及最后一处本人凭证均实测。已打开最终复核页与关键状态拼图人工目视，未冒称微信真机或替用户做最终审美确认。 |
...
A4 本轮起点：f79bb106ef4a758e015fb4895dc4aa0c42455e96；main 为 cc411965dd384e9608a80abf87a9e883eea8294d；开工时 PR #1 仍 Draft/OPEN，沿用原分支/PR，不转 Ready、不合并。
执行入口：WebCodex Windows Runner；Node.js 24.12.0 / npm 11.6.2 / 独立 MySQL 8.4.9 / 本机 Chrome。
A1/A2/A3/A4 状态：本地通过待真机。A4 用户视觉评审提出的地点插画、地图主体/折叠列表、详情层级、主操作、上传与领取分工均已实现；自动截图和本地人工目视复核已完成，用户最终审美确认另行进行。
实际命令：node scripts/prepare-a4-art.mjs；npm run check（32/32）；npm run build；git diff --check；完整 A4 入口使用 TEST_DB_* 指向独立 127.0.0.1:3316、TEST_BROWSER_EXECUTABLE 指向本机 Chrome，A1–A4 共 39 项通过；脚本内仍执行 dotenv-only npm run db:schema。A4 无第二套测试平台或重型地图依赖。
...
前端：最终视觉证据目录 tmp/changqi_a1_test_a86ad91a5a9b；index.html 可直接打开，a4-screenshots.json 记录版本和运行时指纹。44 状态 × 320/390/430 共 132 张整页 PNG，另有 map-only 与关键状态复核拼图；五处逐 key 扫码、p03→p04→p05 切换、动态 N/重排、非顺序完成、缺图兜底、最后一处本人凭证均通过。路线/地标/点击目标同坐标无偏移。
人工目视：已打开最终复核页并查看原参考地图/390px 本轮地图、三档首页和关键 390px 状态拼图；地图主体、弯曲路线、村落层次和标签清楚，五地点主题可区分，上传/未知/成功/本人凭证/工作人员页面层级未发现遮挡或明显串图。此结论不替代用户最终审美选择。
环境样例：.env.example 已补空 STATS_USER/STATS_PASSWORD，并把旧“Nginx Basic Auth”注释改为应用内固定 Basic Auth；未读取或提交真实凭据。
