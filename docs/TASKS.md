...
### 2026-09-25 · 合并前照片替换认证恢复（本地检查通过，未合并/未发布）
...
| PHOTO | 凭证现场照片核查、显式替换与旧库升级 | 保留 A1–A6；只改本人领奖前当前照片 | 原实现/回归/三宽目视及生产迁移通过；9月25日认证恢复本地检查通过、未合并/未发布；新增真机待验 |
...
**当前接续（2026-09-25）：**本轮照片替换认证恢复的代码、测试和本地构建已通过，完整记录见第0节；按原分支正常提交推送并回读最终 HEAD 后，请用户确认 PR #1 最终合并。PR 保持 Draft/Open，不自动转 Ready、合并、触发镜像构建或发布；不为 TASKS-only 变化反复构建。新恢复路径与上次新分享卡片的真机确认分别记录，不能改写既有基础闭环的用户验收。所有忽略的环境、照片和历次证据保留。
