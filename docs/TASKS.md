...
**当前轮：最终现场地图成图直接替换页面地图（2026-09-24，本地回归、构建和生产部署通过，微信真机待验）。** 地图内自带路线、1–5 点位和兑奖处，页面只保留五个透明查看热点；折叠地点列表、扫码资格、照片、领取、统计及正式二维码不变。旧地图和上一镜像均保留，PR 仍为 Draft/Open，未转 Ready 或合并。
...
**当前接续（2026-09-24）：**最终现场地图源码 `7e792a31185ad2255e88bdeda53e421bfba88f95` 已由 Actions run `35962178962` 构建为 `changqi-checkin-h5:7e792a31185a-35962178962-1` 并部署至现有生产 app 容器；回环/公网只读检查、最终 WebP、数据库健康、持久上传挂载和容器重启均通过。PR 仍 Draft/Open，部署后的 TASKS 文档提交不重新构建。下一步由用户在微信真机检查首页地图、五个透明热点、地点详情、扫码/相册/拍照与领取；真实操作结果和纸样仍待记录。
