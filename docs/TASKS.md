...
- 仓库：`Jonoka/changqi-checkin-h5`；本轮原图插画恢复起点为 Windows/PR #1 实际 head `bf2d7345024b27c8a64afbb1571e100c3a43ee01`，与用户审阅基准一致。最新 `main` 为 `cc411965dd384e9608a80abf87a9e883eea8294d`；PR #1 Draft/OPEN、未合并，Review 为空、无其他开放 PR，继续 `feat/a1-wechat-scan`。开工完整 porcelain 为空；本轮修改限素材提取/生成门禁、测试与说明。最终 HEAD 以 Git/PR 回读为准，不写自引用 SHA。
- 初始化历史：`c5bd5888d993984e8d7cfcbd42e5c92416070d6f` 建立仓库；文档基线随后提交；`5e0a67d` 已补入五张视觉原图。当前 HEAD 以工具或 Git 实际读取为准，不写自引用 SHA。
- 视觉原图：五张 PNG 的 SHA/941×1672 尺寸逐一核对，已实际查看像素且未覆盖。旧 `village-art.mjs` 只保留历史，`prepare-a4-art.mjs` 已不再读取它。地图/古井先行素材检查未通过，页面未换图；当前仍为上一版 6 张 WebP、共 79,248 字节，不算本轮恢复成果。新提取 7 张原生 PNG 输入（含 1 张无字井身片段）共 2,614,709 字节，仅供编辑/检查，不在 runtime 引用。
- 保留已确认布局与上一轮 UI：首页/地点隐藏 CQ；凭证/记录/派发保留真实编号；主操作在地图前；提示随扫码/完成状态变化；身份加载/失败/授权互斥。`web/`、`server/`、`config/`、`db/`、原 PNG、锁文件、正式二维码与微信配置相对起点无改动；未恢复本人修饰、拍照提示、就绪文案或标题框。插画恢复单独未完成，不能沿用旧 A4 视觉通过结论。
- 本轮实际验证：`npm run check` 配置/HTTP + 53/53 Node、`npm run build`、`git diff --check` 通过；新增 11 项来源/逐像素裁切/母版/缺项停止/手工文件保护测试。首次 check 因 Windows Sharp 文件路径缓存导致临时夹具 EBUSY，改读 Buffer 后全部重跑通过。独立 MySQL 8.4.9:3319 + 实际 Chrome 完整 `npm run test:a4:mysql` 39/39；135 个 320/390/430 状态组合实时回归。只新截未换图的 home@390、p03-view@390、1 张首屏及 A2/A3 必要 6 图，共 9 PNG，不是新插画验收；微信网络/SDK 模拟，照片为生成夹具。
...
- Windows 工作区已实际核对并开发，保留既有忽略文件、照片、二维码、视觉原图压缩包及历史截图。本轮只读确认运行容器及发布元数据，未读取完整环境或凭据、未做生产写入。服务器已经部署；微信真机、真实照片和领取操作仍未验证。
...
| A4 | 保留布局/UI，恢复原 PNG 插画 | A2、A3 | 进行中：素材关阻塞；旧运行逻辑回归通过，新插画未接入/未验收 |
...
### [ ] 原 PNG 插画恢复 · 2026-09-23（素材关阻塞，未换图/未发布）
...
| AC-12 | 原图恢复素材关阻塞；布局/UI 保留，新插画未验收 | 原 PNG 与 7 张裁图已目视，地图/古井无合格完整无 UI 母版，不接入占位图。未换图 runtime 的 135 组合通过，新截 2 整页+1首屏+6必要图；不将旧运行回归或生成式示意板算成客户视觉通过。 |
...
**当前接续（2026-09-23）：**`feat/a1-wechat-scan` / 原 Draft PR #1，从 `bf2d7345…` 整理原图恢复，未换图。先补“无 UI 地图＋完整古井”实际图片母版，检查 `assets/illustrations/restoration/index.html`，无需重交原图。check 53/53、build、独立 MySQL/Chrome 39/39、diff 检查通过；UI/业务保留，新素材视觉关未通过。素材合格后再做新比例/热点、其余详情和受影响截图；不发布占位版。线上仍为 f14430b7039d…，不重复初次部署或更换地点码。
...
A1–A5 本地业务与已确认布局/UI 不重建；客户最新插画变更单独待素材及页面验收，不能沿用历史视觉通过结论。系统已部署可访问，本轮未换图、未发布；真实微信、现场照片/领取及纸样仍需单独验证。
