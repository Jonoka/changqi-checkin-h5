...
**当前轮：地图现场方位/线路修正（2026-09-24，待本地完整回归/发布）。** 用户提供现场方位参考图；不替换六张栅格插画，保留现有地图底图，只将 p01–p05 的 HTML 点位、路线线层和“兑奖处”位置按参考图重新标定。点位沿用既有 displayOrder 对应打卡点①–⑤；兑奖处只做非交互位置标记，不新增插画、数据库字段、领取入口或强制打卡顺序。路线为行走参考，不改变任意顺序打卡和扫码资格规则；正式地点二维码/域名不变。用户已确认此前部署后的 iOS/Android 微信授权、扫码、现场照片上传、领取和纸样试扫通过；本轮地图改动仍需最终页面回归并重新发布后才算线上生效。
...
| AC-13 | Skill 验证完成：最新版本构建/冒烟/下载/服务器部署通过，微信真机未验 | 生产禁模拟、开发身份不可转正式、MySQL 会话/照片/领取实际进程重启恢复已回归。项目级 Skill 校验通过；Actions run `35844751374` / attempt 1、源码 `953291f87e544d301381673edbb46d4d457df6d4`：linux/amd64 镜像构建、临时 MySQL schema、`/health`、`/api/activity`、首页、`/q/p01`、无效领取 API 和生产模拟拒绝均通过；镜像内 Node `v24.21.0`、npm `11.19.0`，基础 digest 为 `sha256:5cbc7caba8c2c0f0bca675d1b61b9f2857e1cf1853c6164ee9dd409501a936e7`。artifact `changqi-image-953291f87e54-35844751374-1` 已下载到 `tmp/release-35844751374`，归档 SHA-256 为 `d8cab91ebcdf8ccf00dedc3e4d30cf59d3e8656565dfc7a3c39829acf3c201d3`；已上传并在服务器 `docker load` 成功。项目库幂等建表成功；容器 `changqi-checkin-h5:953291f87e54-35844751374-1` 运行于 `127.0.0.1:3002`，回环和公网 HTTPS `/health` 返回 database connected；Nginx HTTPS 反代、HTTP 301、最新活动接口/地点引导/插画、未登录 401、无效领取码 404 和 `/stats` Basic Auth 均通过，容器重启后仍健康。旧镜像与环境备份保留，可回退；微信真机、真实照片/领取和纸样仍未验证。 |
...
**当前接续（2026-09-23）：**六图 PNG 已实际提交，原 Draft PR #1 / feat/a1-wechat-scan 继续；新地图/五处详情/列表已接入，check 62/62、build、隔离 MySQL/Chrome 39/39、最终页面目视通过。项目级 `.agents/skills/changqi-release/SKILL.md` 已由 `quick_validate.py` 校验通过。用户明确授权对最新提交 `953291f...` 构建部署；该 Skill 驱动 Actions run `35844751374` 全部通过，artifact 已校验并替换生产容器，HTTPS、关键只读接口和容器重启检查通过。旧镜像/环境备份保留；微信真机、真实照片/领取和纸样仍待验。
