...
- 仓库：`Jonoka/changqi-checkin-h5`；本轮上线后 UI 小修起点为 Windows/PR #1 实际 head `6293f23520aa0c9160faf0d0f8c34bf33ffafb28`，与用户审阅基准一致。`main` 为 `cc411965dd384e9608a80abf87a9e883eea8294d`；PR #1 Draft/OPEN、未合并，Review/评论为空、无其他开放 PR，继续 `feat/a1-wechat-scan`。开工完整 porcelain 仅 ` M docs/TASKS.md`，已有首次部署记录原样保留并按新证据纠正旧摘要；无未跟踪文件。最终 HEAD 以 Git/PR 回读为准，不写自引用 SHA。
...
- 已确认：A1–A3 本地业务、A2 连续失败专项及 A4 原视觉确认保持有效。本轮仅修改 App/IdentityStatus/ClaimPanel/CSS 的展示：首页/地点隐藏 CQ；领取凭证/记录与派发页保留真实编号；首页进度与刷新同行、主操作在地图前；地点提示依据真实状态；身份加载/失败/授权互斥。无插画重做；无拍照提示、本人修饰、就绪文案和标题框；未改服务端、身份切换 key、扫码/上传/领取恢复逻辑、依赖、活动配置和正式二维码。
- 本轮实际验证：`npm run check` 配置/HTTP + 42/42 Node 测试、`npm run build`、完整 `npm run test:a4:mysql` 39 项通过。Windows Node 24.12.0/npm 11.6.2，新建独立回环 MySQL 8.4.9:3318，实际 Chrome 153.0.8010.52/Vite、真实文件/SQL/应用重启；微信网络/SDK 明确模拟，照片为生成夹具。135 个状态/宽度组合实时检查，新截 23 张受影响整页 + 10 张首屏 + 6 张既有 A2/A3 关键图，共 39 PNG；未复用旧指纹/截图。
...
| AC-12 | A4 原视觉已确认；本轮 UI 本地通过、待发布/真机 | 新运行时指纹 5a3e3b27…；135 个 320/390/430 状态组合、首页首屏三类操作、CQ 范围、状态文案和身份恢复通过。本轮新截 39 PNG，未复用旧截图；未声称本轮逐图目视或用户确认。 |
...
**当前接续（2026-09-23）：**UI 小修在 `feat/a1-wechat-scan` / 原 Draft PR #1，起点 `6293f235…`。check 42/42、build、隔离 MySQL/Chrome 39/39 通过；新证据目录 `tmp/changqi_a1_test_69c9874b22b0`，详见本轮小修记录。线上仍为 `f14430b7039d…` 镜像，未发布本次 UI。下一步为本轮版本审阅/授权发布及真实微信复核，不重复初次部署、不更换正式地点码。
...
A4 原视觉与 A1–A5 本地结果不重复列为缺项；本轮 UI 另有新源码与新截图证据。系统已部署可访问，本轮更新尚未发布；真实微信、现场照片/领取及纸样仍需单独验证。
