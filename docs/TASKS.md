...
**v1.6 · 2026-09-24 · 网页端 / 本地 Agent 可接续，单次顺序执行**
...
**当前轮：地图现场方位/线路修正（2026-09-24，已回归并发布）。** 用户提供现场方位参考图；不替换六张栅格插画，保留现有地图底图，只将 p01–p05 的 HTML 点位、路线线层和“兑奖处”位置按参考图重新标定。点位沿用既有 displayOrder 对应打卡点①–⑤；兑奖处只做非交互位置标记，不新增插画、数据库字段、领取入口或强制打卡顺序。路线为行走参考，不改变任意顺序打卡和扫码资格规则；正式地点二维码/域名不变。源码 `b64460743205033fefa33adff2321013a87404b0` 已通过本地 `npm run check`（62/62）、`npm run build` 和 `git diff --check`，并由项目级 `changqi-release` skill 构建发布；微信真机与纸样仍单独待验。
