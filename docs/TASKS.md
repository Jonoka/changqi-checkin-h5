...
- 仓库：`Jonoka/changqi-checkin-h5`；A4 起点为 Windows/远端 `feat/a1-wechat-scan` 的 A3 提交 `19ff851a7227f9d8eb92c9e28f3832013713c56d`，`main/origin/main` 为 `cc411965dd384e9608a80abf87a9e883eea8294d`。本轮提交前再次实际 fetch/读取 PR #1：仍 Draft/OPEN、未合并、无新 Review，远端未出现额外提交。开工完整 porcelain 无既有修改或未跟踪文件；沿用原分支/PR，不自动合并。
- 初始化历史：`c5bd5888d993984e8d7cfcbd42e5c92416070d6f` 建立仓库；文档基线随后提交；`5e0a67d` 已补入五张视觉原图。当前 HEAD 以工具或 Git 实际读取为准，不写自引用 SHA。
- 视觉原图：五张 PNG 均保留，SHA-256 与既有 manifest 再次匹配。已查看五图合并预览；A4 只从 home/checkin 的无文字插画区域裁出 `village.webp`（650×310）与 `lane.webp`（640×265），共 118230 字节；不是整张参考图铺底。裁切来源、坐标与源图 SHA 固定在 `scripts/prepare-a4-art.mjs`，不覆盖原图。
- 本轮推进 A4：绿色/米白/暖黄色村落插画、首页真实进度、可查看的示意地图/地点列表、地点上传与成功状态、本人领取、公众号引导/授权错误、匿名派发和人数页的统一视觉。标题、地名、进度、照片及领取码均由真实元素/接口驱动；未加入手机框、小程序胶囊、装置业务或后台。A1–A3 服务端业务规则不变。
- 本轮实际验证：`npm run check`（配置/HTTP + 27 项 Node 测试）、`npm run build`、`npm run test:a4:mysql` 的 38 组 A1–A4 MySQL/HTTP/Chrome 检查通过。含已关闭的 A2 连续核对失败、A3 两路并发/统计保护和真实应用重启。A4 在 390/430px 留存 24 种状态、48 张整页 PNG，校验可见按钮触控尺寸、图片加载与无横向溢出；另验证 320px、减少动画、真实指针地图点击/标题聚焦和配置变化后地图/列表/N 同步。微信/故障注入为模拟，数据库/文件/页面真实执行。
...
- 当前接续：A1–A4 本地通过待真机，连续失败专项没有重新变为待补测。审阅 A4 页面后进入 A5 的剩余回归与外部条件核对，不重建业务链路。A6.1 历史 artifact 待办独立；run `35587927134` 当时存储额度失败，当前额度未复核，本轮没有请求镜像、删除其他项目产物或部署。
...
**实现与素材：**新增 `VillageMap.vue`，地图节点与列表均由 points/completedKeys 生成，只发出查看事件，不调用扫码或打卡写入。保留原生 file input 与现有状态机，成功提示仅在服务器确认完成后出现；保留真实 QR、两次确认、未知结果锁定、匿名派发与统计凭据保护。共享视觉使用本地字体/裁切插画，不请求外部字体或二维码服务。插画和路线明确为概念/游览示意，不能当实景测绘；未新增所谓现场照片素材。

**实际画面证据：**`tmp/changqi_a1_test_e5384f5564d8/a4-screenshots.json` 与同目录 `a4-*-390.png` / `a4-*-430.png` 为最终功能代码的 48 张状态图：首页、地点查看、扫码就绪、选图、保存中、失败/结果不明、成功、待领取/二次确认/已领取、派发、无效凭证、未登录/身份加载失败、引导/授权错误、关闭活动及统计。已对首页、照片成功、待领取、公众号引导做合并缩略预览目视检查；测试夹具照片是生成图片，不是假称现场拍摄。图片与截图保留在本地忽略目录，不提交测试游客照片或领取链接。

**验证与边界：**复用现有 Chrome CDP 和独立 MySQL 8.4.9（3314）；38 组回归通过，无新平台、无新依赖，原 lockfile 未变。完整图校验 390/430px 下图片/按钮/边界；320px 紧凑布局、原生标题焦点与减少动画设置另验。已修正共享统计 CSS 误带 `data-stats-count` 字样造成旧错误页断言命中的问题，保留原断言及实际查询失败不显示 0 的行为。微信授权、相机、相册、物理领取码扫码和服务器仍未实测；本地通过不是用户最终视觉确认或真机验收。
...
| AC-12 | 本地通过，待真机/用户视觉审阅 | A4 实际 Chrome 390/430px 的 24 种状态共 48 张整页图；无横向溢出、可见按钮至少 44px、图片加载通过。地图真实指针点击只查看，标题焦点/320px/减少动画另验。文字、N 和进度均由配置/API 生成，原图仅裁取无文字插画。未冒称真机或用户最终视觉确认。 |
...
A4 已核对起点：19ff851a7227f9d8eb92c9e28f3832013713c56d；main 为 cc411965dd384e9608a80abf87a9e883eea8294d；提交前实际 fetch 再核对未变，沿用 Draft PR #1，最终 HEAD 以实际 Git 读取为准。
执行入口：WebCodex Windows Runner；实际 Node.js 24.12.0 / npm 11.6.2 / 独立 MySQL 8.4.9 / 已安装 Chrome headless。
A1/A2/A3/A4 状态：本地通过待真机。A4 原生页面、裁切插画与关键状态已实现；用户最终视觉审阅、微信与现场派发另验。
实际命令：node scripts/prepare-a4-art.mjs；npm run check（配置/HTTP + 27 项 Node 测试）；npm run build；npm run test:a4:mysql（TEST_DB_* 指向独立3314实例，TEST_BROWSER_EXECUTABLE 指向已有 Chrome，38组 A1–A4）；脚本内 dotenv-only npm run db:schema。临时移开并原样恢复 web/dist 后 HTTP 再次通过；5 张原图 hash 再核对不变；最终 24 状态/48 PNG。A4 没有新增依赖或另一份 lockfile。
MySQL：本轮新建独立 MySQL 8.4.9（127.0.0.1:3314）和随机测试库。保留并回归实际照片保存、两路领取/count=2、12请求并发仅新增1人、活动关闭、真实UPDATE拒绝、统计故障、实际应用进程重启恢复；无SQLite/内存假表替代。
前端：最终截图目录 tmp/changqi_a1_test_e5384f5564d8；24状态在390/430px截图并断言布局/图片/触控尺寸；实际指针地图只查看并聚焦地点标题，配置增加测试点后地图/列表/N同步，320px与减少动画另验。持续失败锁定和两路领取保留。SDK/故障明确模拟，SQL/文件真实执行。
A1/A2 已关闭问题：dotenv建表、API404、构建前后HTTP、Sharp临时文件句柄、multipart提前401、旧进度响应覆盖及连续核对失败专项均保留；本轮不重做已通过的业务实现。残留专项待测措辞已清理。
未验证：用户对最终A4页面的视觉确认；真实公众号权限/凭据、微信文档复核、联调HTTPS/域名，iOS/Android微信拍照/相册/权限/领取码相机识别及现场实物派发；生产统计和容器更新。本轮使用生成图片夹具，插画不等于长岐实景。
...
下一步：审阅A4页面后进入A5剩余回归与外部条件核对；本地无已知业务阻塞，不重建A1–A3、不重新列连续失败专项待补测、不依靠生产部署凑测试。
