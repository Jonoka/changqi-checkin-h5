...
**当前轮：最终现场地图成图直接替换页面地图（2026-09-24，本地回归/目视通过，未构建发布）。** 地图内自带路线、1–5 点位和兑奖处，页面只保留五个透明查看热点；折叠地点列表、扫码资格、照片、领取、统计及正式二维码不变。旧地图和派生资源未覆盖，生产仍运行上一轮源码 `b64460743205033fefa33adff2321013a87404b0`，本轮未触发 Actions、未转 Ready、未合并或部署。
...
**上一轮：地图现场方位/线路修正（2026-09-24，已回归并发布）。** 用户提供现场方位参考图；当时保留旧地图底图，并将 p01–p05 的 HTML 点位、路线线层和“兑奖处”位置按参考图重新标定。源码 `b64460743205033fefa33adff2321013a87404b0` 已由项目级 `changqi-release` skill 构建发布；该实现现由本轮最终成图方案在源码中替代，生产尚未更新。
...
| A4 | 保留布局/UI，接入地点插画与最终现场地图 | A2、A3 | 最终地图本地回归/三宽目视通过；未发布，微信真机待验 |
| A5 | 必要回归、问题修复 | A1–A4 | 本地通过待真机 |
| A6 | 双入口构建、本地中转、授权部署与二维码 | 构建/部署依各自授权；纸样不等镜像发布 | 进行中：A6.1/A6.2 最新版本构建、校验、服务器更新与 HTTPS 检查完成；A6.3 样张/解码完成，微信真机/纸样待验 |
...
| AC-12 | 最终地图本地/目视通过；待用户确认与微信真机 | 完整现场地图 PNG/WebP 已接入，页面无额外可见路线、兑奖处、编号或地点标签；五个透明热点、折叠列表、动态增删、无位置地点兜底、扫码资格不变及 320/390/430px 点击/比例/首屏均通过。证据 `tmp/changqi_a1_test_9615c4129fc2/`；未发布，不冒称微信真机已验。 |
| AC-13 | Skill 验证完成：最新版本构建/冒烟/下载/服务器部署通过，微信真机未验 | 项目级 `changqi-release` skill 校验通过；Actions run `35950810377` / attempt 1、源码 `b64460743205033fefa33adff2321013a87404b0`：linux/amd64 镜像、临时 MySQL schema、前端构建和 smoke checks 全部通过。artifact `changqi-image-b64460743205-35950810377-1` 已下载到 `tmp/release-35950810377`，归档 SHA-256 为 `edd606c70249dbb699f543b144a7773099dcdbd204961aceb5288942d3069824`；已上传并在服务器 `docker load` 成功，镜像 ID 为 `sha256:b193ec7fa0e5410e5945b6f5ef5b8b7e7a84870498fad18a522061b273c031f4`。项目库幂等建表成功；容器 `changqi-checkin-h5:b64460743205-35950810377-1` 运行于 `127.0.0.1:3002`、`network_mode: bridge`，回环/公网 HTTPS `/health` 返回 database connected；`/api/activity` 五点、`/q/p01`、WebP、未登录 401、无效领取码 404、`/stats` 401、HTTP 301、容器重启后健康和 Windows 外部 HTTPS 检查均通过。回滚备份保留；微信真机、真实照片/领取和纸样仍未验证。 |
...
**当前接续（2026-09-24）：**当前分支已完成最终现场地图成图替换，本地 `npm run check` 62/62、`npm run build`、`git diff --check`、隔离 MySQL/实际 Chrome `npm run test:a4:mysql` 45/45 通过；三宽证据目录为 `tmp/changqi_a1_test_9615c4129fc2/`。本轮不请求 Actions、不发布；生产继续运行 `b64460743205033fefa33adff2321013a87404b0` 的上一轮叠加地图版本。下一步仅在另行获得发布授权后按 DEPLOY 构建和更新；微信真机、真实照片/领取和纸样仍待验。
