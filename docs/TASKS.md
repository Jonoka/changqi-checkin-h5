...
| PHOTO | 凭证现场照片核查、显式替换与旧库升级 | 保留 A1–A6；只改本人领奖前当前照片 | 本地实现/回归/三宽目视及生产迁移通过；新增真机待验 |
...
**最新接续 · 分享完整标题修正（2026-09-24）：**生产已运行修复源码 `a851773b3571796d0e902c365560284b14559b82` 对应的 Actions run `36006086113` / attempt 1 镜像；完整发布证据见第0节。源码、Actions `headSha`、`build-info.json`、运行镜像、公网 HTML 和实际引用 JS 已闭环一致；线上标题为“2026三水区芦苞镇长岐古村黄金节庆影视游园季活动”，页面可见活动名仍为“长岐村漫游打卡”。

每个执行任务结束时更新顶部状态、任务表、验收表和下面的当前摘要。避免重复长篇历史；需要的命令输出可放不含敏感数据的测试报告，并在此引用。

**当前接续（2026-09-24）：**下一步只提交推送本次 TASKS 发布记录并同步 PR 正文，不为 docs-only 提交重新构建；PR 保持 Draft/Open。请在 iOS/Android 微信关闭旧活动页后重新进入正式首页，等待 SDK ready，再从首页、地点页、`/#claim` 和已有匿名 `/r/:claimCode` 新发卡片，确认完整标题、新海报和点击统一落到公开首页；不要只转发旧聊天卡片。真机结果与代码、构建、线上资源状态分开记录。
