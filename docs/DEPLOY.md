...
**v1.4 · 2026-09-21 · 操作约定，尚未实现工作流或执行部署**
...
A6.1 实现 `.github/workflows/build-image.yml`，名称可用 `Build application image`，只保留一套检查和镜像导出步骤。触发约定如下，**这只是待实现片段，不是本轮已提交的工作流**：[G1][G2]
...
2026-09-21 本轮只更新约定：没有创建工作流、发起构建、生成镜像或连接服务器。当前 GitHub App 的读取权限不代表已实测 Actions 构建权限/额度。后续事实都记在 TASKS，不在本文件累积重复运行日志。
