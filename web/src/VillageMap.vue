...
<template>
  <section class="village-map" aria-labelledby="map-title">
    <div class="section-heading"><div><p class="eyebrow">沿着风景，慢慢走</p><h2 id="map-title" tabindex="-1">长岐漫游地图</h2></div><span class="paper-tag">现场地图</span></div>
    <p class="map-caption">点击地图中的 1–5 标记查看地点</p>
    <div class="map-canvas" :style="{ aspectRatio: `${layout.width} / ${layout.height}` }" :data-placement="layout.placement">
      <img class="map-scenery" :src="layout.image" alt="长岐村水乡漫游地图，包含五个地点、游览路线与兑奖处位置" :width="layout.width" :height="layout.height" decoding="async" />
      <ol class="map-points">
        <li v-for="node in layout.nodes" :key="node.point.key"
    ...
    <p class="muted map-footnote">地图内路线仅作游览参考，不限制打卡顺序；点击地图不会取得扫码资格。</p>
  </section>
</template>
