...
<template>
  <section class="identity-block" :class="{ 'progress-strip paper-card': !compact, 'detail-progress': compact }" aria-live="polite" aria-label="漫游进度">
    <template v-if="me">
      ...
      <div v-if="!compact" class="progress-footer"><span>{{ me.userLabel }}</span><button type="button" class="text-button" :disabled="locked" @click="emit('refresh')">刷新状态</button></div>
    </template>
    <template v-else>
      <p v-if="identityState === 'loading'" role="status">正在恢复身份…</p>
      <p>登录后显示进度 · 共 {{ activity.points.length }} 个地点</p>
