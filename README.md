...
面向“印象芦苞”微信公众号的轻量照片打卡活动：微信扫一扫地点码 → 自动识别当前用户 → 对应地点上传现场照片 → 完成全部地点 → 现场领取 → 标记领取人数；也可从公众号进入后使用页内扫一扫。
...
真实微信使用本地填写的 `WECHAT_APP_ID`、`WECHAT_APP_SECRET`，先确认授权域名、JS 接口安全域名、接口权限及服务端网络条件；不改变公众号菜单、消息服务或第三方托管配置。OAuth 错误页提供手动重试，接口未登录返回 JSON 401。微信内直接打开有效 `/q/p01` 每次经现有 snsapi_base 重新识别当前微信用户，固定 `/auth/callback` 保存会话和当前地点后跳 `/#point/p01`，无需关注或二次扫码；微信外游客入口仅显示使用微信引导，旧 Cookie 不例外，且不依赖数据库。无效地点不授权，失败停止跳转并保留安全地点供手动重试。活动关闭只识别/查看原记录，不授新上传资格。页内扫码继续 `/api/scan`，识别地点但不增加进度。地点查看使用 hash 路由，不授予扫码资格；扫码后可选择现场照片、预览、重选并提交。完成全部地点后可出示本人领取码、确认已领取，或由持码者在派发页确认；首次时间与渠道只记录一次。
...
`/stats` 通过既有固定凭据只读显示参与、完成、已标记领取、完成未领取，辅助显示已识别身份、有效照片记录、各地点及北京时间每日首次事件。手动刷新，数据库/配置失败报错，不显示假 0。相同认证保护 `/stats/export/summary.csv`、`/stats/export/points.csv`、`/stats/export/daily.csv`，仅下载聚合数据；未知报表404，未配置凭据503。

领取码仍由本地依赖从本人 `claimUrl` 生成，不发送给第三方二维码服务；二维码与链接仅向派发人员出示。开发演示必须使用独立测试库，不能将模拟领取记录混入生产统计。

### 一次性受控照片资料导出

这是命令行工具，不是后台或网页相册。必须由负责人确认私有输出父目录和显式 `DB_HOST/DB_PORT/DB_NAME/DB_USER/DB_PASSWORD/UPLOAD_DIR` 环境；不自动读取 `.env`，不使用游客 Cookie，不建库/迁移/停止活动。命令本身不代表获准操作生产。完整操作及指定镜像示例见 [DEPLOY](docs/DEPLOY.md#一次性只读照片资料导出未来授权后使用)。

```sh
npm run export:activity -- --help
npm run export:activity -- --out "/private/activity-exports/活动导出-新批次" --scope participants --dry-run
npm run export:activity -- --out "/private/activity-exports/活动导出-新批次" --scope participants
```

Windows 使用自己的绝对路径，例如 `D:\\private-exports\\活动导出-新批次`。父目录必须预先由负责人设置为私有；批次目录不能存在，位于源码、公开目录及源 uploads 之外。dry-run 不创建导出文件。scope 默认 participants，也支持 completed/claimed；三个统计CSV仍是全活动总览，只有游客进度/照片清单随 scope 选择。照片逐张按已提交引用复制 JPEG 原字节，每张最多3次，记录实际版本/时间/SHA-256。缺图不撤销历史完成，只写异常。退出码0完整、2部分异常、1致命失败；以 `export-info.json` 和清单核对，不用目录存在推断成功。

可选私有环境 `STATS_EXCLUDE_CQ_FILE` 指向负责人确认的规范 CQ JSON 数组（例如 `["CQ000123"]`）；默认空，不猜测测试用户。页面/CSV/CLI使用同一份文件，指定文件失败或格式错误必须报错。名单不进 Git、客户包或镜像。

统计采用短只读一致快照；日汇总先聚合每人完整记录，再按首次参与/推导完成/领取日期分组（+08:00）。读取超过200,000条连接结果会明确报错，绝不截断后冒充完整报告。元数据内存为 O(身份+有效记录)，照片逐张64 KiB缓冲，不整批装入内存。每个SQL数据查询有15秒超时；不在复制期间持事务。活动期间照片可晚于统计快照，正式收尾应另行授权停止写入并等待在途结束。

资料包只有内部记录/核查用途，不是宣传授权；不包含 OpenID、领取码/私有链接、Cookie、绝对源路径、密钥、完整排除名单或数据库备份。下载到负责人私有电脑后再用系统工具压缩；不上传真实照片至 Git、Actions artifact 或聊天。不会自动删除源照片或旧批次。
...
游客说明：使用微信扫一扫现场地点二维码，打开页面并上传现场照片。也可从‘印象芦苞’公众号进入活动，使用页面内扫一扫。五个原载荷和码图不变；旧纸样包保留，外围旧说明按本版更正，不要求重新印码图。可单独附上 [地点码说明更正](docs/point-qr-entry-correction.md)。微信真机结果与本地 OAuth/SDK 模拟分开记录。
